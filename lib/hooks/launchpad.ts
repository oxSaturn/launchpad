...
const getIsWhitelisted = async (address: string | undefined) => {
  const whitelist = await fetch(
    "https://anythingyouwantintheworld.com/DMT_whitelistsale.json"
  );
  const whitelistJson = (await whitelist.json()) as string[];
